...
const Profile = () => {
  ...
  // state for conditional messages
  ...
  useEffect(()=>{
    ...
    if (storedUser && storedUser.email) {
      setUser({
        ...
        password: '' // Do not store password in localStorage in production
      });
      ...
      .then(data => {
        if (data.error) {
          ...
          setUser({
            name: data.name,
            email: data.username,  // or data.email, depending on your endpoint
            password: data.password // Only for demonstration! Avoid in production.
          });
        }
      }).catch(error => {
      ...
      >
        ...
        >
          ...
          >
            ...
            />
            <h2 className='mt-3 text-black'>{user.name || "Your Name"}</h2>
          </div>
              ...
              <div className='d-flex align-items-center justify-content-center' style={{width: "100%", gap: "1rem"}}>
                <div className='d-flex flex-column align-items-center' style={{width: "100%"}}>
                  ...
                  <label htmlFor="name" className="form-label" style={{ color: "#CFB991" }}>Name:</label>
                    ...
                    {/* Password Field */}
                    <div className="mb-3 w-100">
                      <label htmlFor="password" className="form-label" style={{ color: "#CFB991" }}>Password:</label>
                      <input 
                        type="password" 
                        id="password"
                        value={password} 
